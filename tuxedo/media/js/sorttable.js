...
sorttable = {
        ...
        for (var i=0; i<headrow.length; i++) {
            ...
            for (var k=0; k<headrow[i].childNodes.length; k++) {
                if (headrow[i].childNodes[k].tagName == 'A') {
                  ...
                  headrow[i].childNodes[k].onclick = function() { return false; };
                }
            }
  ...
  sort_table: function(cell) {
      ...
      for (var j=0; j<rows.length; j++) {
          if (j % 2)
              rows[j].className = rows[j].className.replace('even',
                                                            'odd');
          else
              rows[j].className = rows[j].className.replace('odd',
                                                            'even');
  ...
  reverse_table: function(cell) {
    ...
    for (var i = newrows.length-1; i >= 0; i--) {
        if (i % 2)
            newrows[i].className = newrows[i].className.replace('even',
                                                                'odd');
        else
            newrows[i].className = newrows[i].className.replace('odd',
                                                                'even');
