...
# IP address regex
my $ipregex =
qr{^([01]?\d\d?|2[0-4]\d|25[0-5])\.([01]?\d\d?|2[0-4]\d|25[0-5])\.([01]?\d\d?|2[0-4]\d|25[0-5])\.([01]?\d\d?|2[0-4]\d|25[0-5])$};
...
{
    $location_sql =
qq{SELECT mirror_locations.* FROM mirror_locations INNER JOIN mirror_products
    ...
    if ( defined( $ARGV[1] ) ) {
        if ( $ARGV[1] =~ /^\d+$/ ) {
            $mirror_sql =
              qq{SELECT * FROM mirror_mirrors WHERE active='1' AND id=$ARGV[1]};
        }
        else {
            $mirror_sql =
qq{SELECT * FROM mirror_mirrors WHERE active='1' AND (baseurl LIKE }
              . $dbh->quote( '%' . $ARGV[1] . '%' )
              . qq{ OR name LIKE }
              . $dbh->quote( '%' . $ARGV[1] . '%' ) . ")";
        }
    ...
    $location_sql =
qq{SELECT mirror_locations.* FROM mirror_locations INNER JOIN mirror_products ON mirror_locations.product_id = mirror_products.id WHERE mirror_products.active='1'};
    $mirror_sql =
      qq{SELECT * FROM mirror_mirrors WHERE active='1' ORDER BY name};
}
my $update_sql =
...
# Unhealthy, not fatal.
    if ( $domain !~ m($ipregex) && !$netres->query($domain) ) {
        log_this
"DNS resolution for $mirror->{mirror_name} FAILED!  Moving on to next mirror.\n";
...
# Unhealthy, not fatal.
    if ( $mirrorRes->{_rc} >= 500 ) {
        if ( $mirrorRes->{_rc} == 500 ) {
            log_this "$mirror->{baseurl} sent no response after "
              . $ua->timeout()
              . " seconds!  Checking recent history...\n";
            $failed_mirror_sth->execute( $mirror->{id} );
            $log_sth->execute( $start_timestamp, $mirror->{id}, '0',
                $mirror->{rating}, "No response" );
            $getlog_sth->execute( $mirror->{id} );
            my ( $prevweight, $prevactive ) = ( $mirror->{rating}, 1 );
            my $dropit = 1;
            while ( my ( $weight, $active ) = $getlog_sth->fetchrow_array ) {
                log_this
                  "**** weight $weight active $active for $mirror->{baseurl}\n";
            ...
            if ($dropit) {
                log_this
"**** $mirror->{baseurl} Weight Drop Pattern matched, weight will be dropped 10%\n";
                $newweight =
                  $mirror->{rating} - int( $mirror->{rating} * 0.10 );
                log_this
"**** $mirror->{baseurl} Weight change $mirror->{rating} -> $newweight\n";
                $updaterating_sth->execute( $newweight, $mirror->{id} );
                $logratingchange_sth->execute( $mirror->{id}, $mirror->{name},
                    "Changed rating to $newweight." );
            }
            else {
            ...
            log_this "$mirror->{baseurl} returned error "
              . $mirrorRes->{_rc} . ".\n";
            $failed_mirror_sth->execute( $mirror->{id} );
            $log_sth->execute( $start_timestamp, $mirror->{id}, '0',
                $mirror->{rating}, "Bad response" );
        }
