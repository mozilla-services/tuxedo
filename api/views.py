...
def location_add(request):
    ...
    except (Product.DoesNotExist, OS.DoesNotExist) as e:
        return xml.error(e)
...
def location_modify(request):
    ...
    try:
        ...
        location.path = path
        location.save()
    except (Product.DoesNotExist, OS.DoesNotExist, Location.DoesNotExist) as e:
        return xml.error(e)
    ...
    if not form.is_valid():
        if 'alias' in form.errors:
            if 'required' in form.errors['alias'][0]:
                return xml.error('alias name is required.',
        ...
        if 'related_product' in form.errors:
            if 'required' in form.errors['related_product'][0]:
                return xml.error('related_product name is required.',
        ...
        return xml.error('There was a problem validating the data provided',
